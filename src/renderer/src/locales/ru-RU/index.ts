export default {
  ...
  modal: {
    ...
    adminRequiredDesc:
      'Для режима TUN приложение нужно один раз запустить от имени администратора. Без этих прав приложение продолжит работать через системный прокси, и запрос больше не появится — выдать права можно позже в настройках ядра.',
    restartAsAdmin: 'Перезапустить от имени администратора',
    continueWithoutAdmin: 'Продолжить без прав администратора'
  },
  ...
  profile: {
    ...
    editRules: {
      ...
      loadErrorHint:
        'Сохранение отключено, чтобы не затереть уже добавленные правила. Откройте YAML-редактор, чтобы изменить файл вручную.',
      saveBlocked: 'Сохранение недоступно: правила не загрузились',
